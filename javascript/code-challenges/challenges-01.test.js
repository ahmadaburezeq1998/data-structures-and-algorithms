...
const allUpperCase = (arr) => {
  let newUpperArray=[];
  arr.forEach(element => {
    newUpperArray.push(element.allUpperCase);
  });
...
const greeting = (word) => {
  let newString=word.allUpperCase+'!';
  return newString;
...
const speaker = (words, callback) => {
  ...
  words.forEach(i=>{
    newArrayOfStrings.push(words[i]);
  })
};
...
const addNumbers = (num, arr, times, callback) => {
  times.forEach(arr.push(num));
  arr.addValues(arr,addNumbers);
